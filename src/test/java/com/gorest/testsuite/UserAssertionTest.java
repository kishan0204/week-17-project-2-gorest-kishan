...
public class UserAssertionTest extends TestBase {
    ...
    @Test
    public void test002() {
        response.body("find{it.id==5235}.name",equalTo("Bhilangana Shah"));
    }
    //3. Check the single ‘Name’ in the Array list (Subhashini Talwar)
    @Test
    public void test003() {
        response.body("[1].name",equalTo("Aaryan Nayar"));
    }
    //4. Check the multiple ‘Names’ in the ArrayList (Mrs. Menaka Bharadwaj, Msgr. Bodhan Guha, Karthik Dubashi IV)
    @Test
    public void test004() {
        response.body("name",hasItems("Bhilangana Shah","Hiranmaya Kakkar","Aaryan Nayar"));
    }
    //5. Verify the email of userid = 5471 is equal “adiga_aanjaneya_rep@jast.org”
    @Test
    public void test005() {
        response.body("find{it.id==5227}.name",equalTo("Hiranmaya Kakkar"));
    }
    //6.Verify the status is “Active” of user name is “Ganak Jain”
    @Test
    public void test006() {
        response.body("find{it.name=='Dipankar Khanna'}.status",equalTo("active"));
    }
    //7. Verify the Gender = male of user name is “Niro Prajapat”
    @Test
    public void test007() {
        response.body("find{it.name=='Dipankar Khanna'}.gender",equalTo("female"));
    }
}
